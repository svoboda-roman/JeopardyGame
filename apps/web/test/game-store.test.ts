...
function baseGame(): GameView {
  return {
    ...
    phase: 'lobby',
    players: [
      { id: 'p-host', displayName: 'Host', score: 0, status: 'joined', isHost: true },
      { id: 'p1', displayName: 'P1', score: 0, status: 'joined', isHost: false },
    ],
    board: [
      {
        ref: 'c1',
...
describe('game store reducer', () => {
  ...
  it('judged updates the right player score', () => {
    ...
    store
      .getState()
      .apply({ type: 'judged', playerId: 'p1', verdict: 'correct', scoreDelta: 100, newScore: 100 })
  ...
  it('error message is captured', () => {
    const store = createGameStore()
    store.getState().apply(snap())
    store.getState().apply({ type: 'error', code: 'early_buzz', message: 'too soon' })
