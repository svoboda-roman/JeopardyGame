      ...
      <div className="w-full max-w-md border rounded-2xl p-6 text-center space-y-3">
        ...
        {error && (
          <>
            ...
            <Link to="/" className="underline text-sm">Go home</Link>
          </>
        )}
        {data && (
          <>
            <p className="text-sm uppercase tracking-wider text-muted-foreground">Quiz</p>
            <h1 className="text-2xl font-bold">{data.quizTitle}</h1>
            <p className="text-sm text-muted-foreground">by {data.ownerDisplayName}</p>
