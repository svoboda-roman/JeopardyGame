...
function EditorPage() {
  ...
  if (error || !data) return <p className="p-4 text-sm text-destructive">Quiz not found.</p>
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <header className="flex flex-wrap items-center gap-2 justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button
            variant="destructive"
            onClick={() => {
              if (confirm('Delete this quiz? This cannot be undone.')) deleteMut.mutate()
            }}
          >
            Delete
          </Button>
      ...
      <Board categories={data.categories} questions={data.questions} quizId={quizId} />
    </div>
  )
}
...
function CategoryField({ category, quizId }: { category: Category; quizId: string }) {
  const qc = useQueryClient()
...
function QuestionEditor({
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-10 bg-background/90 backdrop-blur p-4 flex items-center justify-center"
      onClick={onClose}
    >
      <div
        className="bg-card border rounded-2xl p-4 w-full max-w-lg space-y-3"
