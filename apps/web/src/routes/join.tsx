  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.status === 409) {
        ...
        setError(body?.error?.code === 'full' ? 'Game is full.' : 'Game cannot be joined right now.')
        return
      }
      if (!res.ok) {
    ...
    <div className="min-h-screen flex items-center justify-center px-4">
      ...
      >
        ...
        <div className="space-y-2">
          <label className="text-sm" htmlFor="code">Room code</label>
          <input
            id="code"
            required
            minLength={6}
        ...
        <Button type="submit" className="w-full" disabled={submitting || !code || !name}>
