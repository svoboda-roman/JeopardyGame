...
import type { GameView, PlayerView, ServerToClient } from 'server/src/game/protocol.ts'
...
export const createGameStore = () =>
  create<GameStoreState>((set, get) => ({
    ...
    apply(msg) {
      switch (msg.type) {
        case 'snapshot':
          set({ game: msg.game, selfPlayerId: msg.you.playerId, lastError: null })
          return
        case 'player_joined': {
          const g = get().game
          if (!g) return
        ...
        case 'player_left': {
          const g = get().game
          if (!g) return
          const players = g.players.map((p): PlayerView =>
            p.id === msg.playerId
              ? { ...p, status: msg.reason === 'kicked' ? 'kicked' : msg.reason === 'disconnect' ? 'disconnected' : 'left' }
              : p,
          )
          set({ game: { ...g, players } })
          return
        ...
        case 'buzzed': {
          const g = get().game
          if (!g) return
          set({ game: { ...g, phase: 'buzzed', currentPlayerId: msg.playerId } })
          return
        }
        case 'judged': {
          const g = get().game
