...
import type { InternalBoard, InternalQuestion } from '../game/state.ts'
...
async function snapshotQuiz(quizId: string, ownerId: string): Promise<InternalBoard> {
  const q = await db
    .select()
    .from(quizTable)
  ...
  return {
    categories: cats.map((c) => ({
      ...
      position: c.position,
      title: c.title,
      questionRefs: allQs.filter((qq) => qq.categoryId === c.id).map((qq) => qq.id),
    })),
    questions,
  }
}
...
export const games = new Elysia({ tags: ['games'] })
  ...
  .post(
    ...
    async ({ request, body, set }) => {
      ...
      const board = await snapshotQuiz(body.quizId, u.id)

      // Read host display name once (profile preferred, fallback to user.name).
      const profileRow = (
        await db
          .select({ displayName: userProfileTable.displayName, fallback: userTable.name })
          .from(userTable)
          .leftJoin(userProfileTable, eq(userProfileTable.userId, userTable.id))
          .where(eq(userTable.id, u.id))
          .limit(1)
      )[0]
      const hostName = profileRow?.displayName ?? profileRow?.fallback ?? 'Host'
      ...
      for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
        ...
        try {
          const created = await db.transaction(async (tx) => {
            const [g] = await tx
              ...
              .values({
                ...
                hostId: u.id,
                status: 'lobby',
                options: { readDelayMs: body.options?.readDelayMs ?? 3000 },
              })
              .returning()
            if (!g) throw new Error('game insert returned no row')

            await tx.insert(gameSnapshotTable).values({ gameId: g.id, quiz: board })
            await tx
              .insert(gamePlayerTable)
              .values({ gameId: g.id, userId: u.id, displayName: hostName, status: 'joined' })
            return g
          })
          set.status = 201
          ...
          if (isUnique && (constraint === 'game_room_code_uk' || msg.includes('game_room_code_uk'))) {
            continue
          }
          if (
            isUnique &&
            (constraint === 'game_host_active_uk' || msg.includes('game_host_active_uk'))
          ) {
            throw new HttpError(409, 'conflict', 'You already have an active game.')
          }
          throw e
        }
      }
  ...
  .get('/games/:roomCode', async ({ params }) => {
    ...
    const g = rows[0]!
    return { game: { id: g.id, roomCode: g.roomCode, status: g.status, hostId: g.hostId } }
  })
  ...
  .post(
    ...
    async ({ request, params, body, cookie, set }) => {
      ...
      const c = await db
        .select({ n: count() })
        .from(gamePlayerTable)
        .where(and(eq(gamePlayerTable.gameId, g.id), eq(gamePlayerTable.status, 'joined')))
      if ((c[0]?.n ?? 0) >= MAX_PLAYERS_PER_GAME) {
        throw new HttpError(409, 'full', 'Game is full')
      }

      const newGuestToken: string | null = user ? null : (guestToken ?? crypto.randomUUID())
      const [created] = await db
        ...
        .returning()
      if (!created) throw new HttpError(500, 'internal', 'join insert returned no row')
