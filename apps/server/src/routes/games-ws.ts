...
async function authenticate(roomCode: string, request: Request): Promise<{ playerId: string; displayName: string } | null> {
  ...
  if (session) {
    const rows = await db
      ...
      .where(
        and(eq(gamePlayerTable.gameId, await gameIdForCode(roomCode)), eq(gamePlayerTable.userId, session.user.id)),
      )
      .limit(1)
    if (rows.length > 0) return { playerId: rows[0]!.id, displayName: rows[0]!.displayName }
  }

  // Try guest cookie scoped to this room.
  const cookieHeader = request.headers.get('cookie') ?? ''
  const match = cookieHeader.match(new RegExp(`guest_token_${roomCode}=([^;]+)`))
  if (match) {
...
async function gameIdForCode(roomCode: string): Promise<string> {
  ...
  const rows = await db
    ...
    .limit(1)
  if (rows.length === 0) throw new Error(`Unknown game ${roomCode}`)
  const id = rows[0]!.id
...
function intentFor(playerId: string, msg: ClientToServer): Intent | null {
  switch (msg.type) {
    ...
    case 'select_question':
      return { type: 'select_question', actorId: playerId, questionRef: msg.questionRef }
    case 'open_question':
...
export const gamesWs = new Elysia().ws('/ws/games/:roomCode', {
  async open(ws) {
    ...
    const auth = await authenticate(roomCode, (ws.data as { request: Request }).request)
    if (!auth) {
    ...
    sockets.set(ws.raw as object, { roomCode, socketId, playerId: auth.playerId })

    driver.registerSocket(
      { socketId, ws: { send: (s: string) => ws.send(s) }, playerId: auth.playerId },
  ...
  async message(ws, raw) {
    ...
    try {
      ...
      if (!msg || typeof msg !== 'object' || typeof msg.type !== 'string') throw new Error('bad shape')
    } catch {
    ...
    try {
      ...
      if (e instanceof GameError) {
        ws.send(JSON.stringify({ type: 'error', code: e.code, message: e.message }))
      } else throw e
    }
  },
  ...
  close(ws) {
    ...
    void getOrLoadRoom(meta.roomCode).then((driver) => driver?.unregisterSocket(meta.socketId))
  },
})
